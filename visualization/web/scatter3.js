  ...
  chart.tooltipContent(function(groupName, x, y) {
	  ...
	  for (ii = 0; ii < loadeddata.length; ii++) {
		  if (loadeddata[ii].x == x && loadeddata[ii].y == y) {
			  return loadeddata[ii].label + "(" + loadeddata[ii].openCount + " open pull requests)";
		  }
	  }
	  return "unknown";
  });
...
function myRealData() {
    ...
    for (ii = 0; ii < mydata.repos.length; ii++) {
    	loadeddata.push({
    		...
    		openCount: mydata.repos[ii].pullRequests.openCount
    	})
    	data[0].values.push({
    		label: mydata.repos[ii].name,
    		x: mydata.repos[ii].forks,
    		y: mydata.repos[ii].stars,
    		size: mydata.repos[ii].pullRequests.openCount,
    		shape: "circle"
        });
    }
