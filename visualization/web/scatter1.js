  ...
  chart.tooltipContent(function(groupName, x, y) {
	  ...
	  for (ii = 0; ii < loadeddata.length; ii++) {
		  if (loadeddata[ii].x == x && loadeddata[ii].y == y) {
			  return loadeddata[ii].label + "(" + loadeddata[ii].numContributors + " contributors)";
		  }
	  }
	  return "unknown";
  });
...
function myRealData() {
    ...
    for (ii = 0; ii < mydata.repos.length; ii++) {
    	loadeddata.push({
    		...
    		numContributors: mydata.repos[ii].numContributors
    	})
    	data[0].values.push({
    		label: mydata.repos[ii].name,
    		x: mydata.repos[ii].forks,
    		y: mydata.repos[ii].stars,
    		size: mydata.repos[ii].numContributors,
    		shape: "circle"
        });
    }
